...
%% List of Reds
Reds = zeros(MaxRedNum,17); % initialize boids matrix
...
Reds(:,4:5) = 70; %200*(2*rand([BoidsNum,2])-1); % set random velocity
...
%% List of Blues
Blues = zeros(MaxBlueNum,17); % initialize boids matrix
%{1-3 position, 4-6 velocity, 7-9 accelaration, 10 maxspeed, 11 maxforce, 12 angle,
% 13 max see ahead (for collision avoidance), 14 max avoid force (collision
% avoidance), 15 hit points, 16 radius-fight, 17 radius-warning
%}
...
Blues(:,4:5) = 70; %200*(2*rand([BoidsNum,2])-1); % set random velocity
