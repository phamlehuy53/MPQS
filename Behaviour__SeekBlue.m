...
while (timeTick < TimeSteps)
    ...
    %% set Target for the first position
    if( kB < BluesNum )
    ...
    for BlueIndex = 1:BluesNum
        ...
        if (dist(Blues(BlueIndex,1:3),ObstaclesB(BlueIndex + 1,1:3 )) > 10 ) % <10 thi dung lai
    ...
    for i=1:RedsNum
        if (goToAttack == 0)
            ...
            if(Reds(i,15)>0)
                ...
                if (makeInformation == tempMatrix) % when all troops are in positions -> attack
                   goToAttack = 1; 
                end
            else
                makeInformation(i)=1;
            ...
            if(Reds(i,15)>0)
                ...
                if (J>0 && dist(Reds(i,:),Blues(J,:))<ShootDistanceB) % if the distance is short enough then shoot
                    ...
                    flk_force=steer_flock(CurrentBoid,Reds,RedsNum);    % else move toward the target
                    avd_force=steer_collision_avoidance(CurrentBoid,1,Obstacles, ObstaclesNum); 
