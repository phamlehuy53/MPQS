...
% cho Blues ?i lung tung
    ...
    %         Blues = updateAtBoundary(Blues,BlueIndex);
            ...
            force =  force_steek*2 + force_wander*4 + force_separation*2;
...
%             ?o?n này cho máy báy bay ra th? bom
         ...
         for i = 1:TankNum
             if(Tank(i,15)>0)
                 ...
                 force = seek_force*0.5+avd_force*0.7;
...
%                             disp(["Distance fighting: ", tmpDist]);
        ...
        %             if(Reds(i,15)>0)  % still alive
            ...
            %% Tank
            for i = 1:TankNum
                if(Tank(i,15)>0)  % still alive
                    [tgIndex, tmpDistTank]=distTargets(Tank(i,:),BluesNum,Blues);  % kho?ng cách ?? g?n thì chuy?n thành BattleFighting
                        if tmpDistTank < RadiusFight
                            disp(["Distance fighting: ", tmpDist]);
                            BattleStatus = BattleFighting;
                        else
        %                     disp(["Distance: ", tmpDist]);
                    ...
                    Tank = updateAtBoundary_Tank(Tank,i);
                    CurrentTank = Tank(i, :);
                    see_force = steer_seek(CurrentTank, [-200, -200, 0, 0]); % Red lao th?ng xu?ng base c?a Blue
...
%             end                
        ...
        for i=1:TankNum
                if(Tank(i,15)>0)
                    Tank = updateAtBoundary_Tank(Tank,i);
                    CurrentTank = Tank(i, :);
                    [J,tmpDistTank]=findTarget(Tank(i,:),BluesNum,Blues);       
                    if (J>0 && dist(Tank(i,:),Blues(J,:))<ShootDistanceTank) % ?? g?n thì b?n
                        ...
                        flk_force=steer_flock(CurrentTank,Tank,TankNum);    % còn không thì t? l?i
                        avd_force=steer_collision_avoidance(CurrentTank,1,Obstacles, ObstaclesNum); 

                        if (J>0)
                            arr_force = steer_arrival(CurrentTank, Blues(J,:));
                        else
